// src/schemas/postmanCollectionSchema.ts
import { jsonSchemaToZod } from "json-schema-to-zod";
import { resolveRefs } from "json-refs";
import * as path from "node:path";
import { writeFile } from "node:fs/promises";

const postmanCollectionJsonSchema = {
  // Paste the entire Postman Collection 2.1 JSON schema here
  // 20240806174107
...
async function main() {
  const { resolved } = await resolveRefs(postmanCollectionJsonSchema);
  const code = jsonSchemaToZod(resolved);

  await writeFile(
    path.resolve(process.cwd(), "src/postman-collection-schema.ts"),
    code
  );
}
